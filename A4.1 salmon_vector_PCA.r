...
#do PCA, get PC1 and PC2 gene list, code modified from Joo Hyun
#Normalization: estimate the effective library size.
dds_sub = dds
#dds_sub = dds
#dds_sub = dds[, grepl("Tissue*", dds$condition ) ]
#dds_sub <- dds[, grepl("Infection*", dds$condition) & (grepl("*Virgin*", dds$condition) | grepl("*UC*", dds$condition))]
#dds_sub <- dds[, grepl("Infection*", dds$condition) ]

#dds_sub$condition <- relevel(dds_sub$condition, ref="Dsuz_Whole_Fly_UC")
